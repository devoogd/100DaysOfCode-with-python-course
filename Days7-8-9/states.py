...
def get_longest_state(data):
    ...
    if type(data) is list:
        length_state_names = [len(state_name) for state_name in data]
        longest = length_state_names.index(max(length_state_names))
        return data[longest]
    if type(data) is dict:
        length_state_names = [len(state_name) for state_name in data.keys()]
        longest = length_state_names.index(max(length_state_names))
        return list(data.keys())[longest]
...
def combine_state_names_and_abbreviations(us_state_abbrev = us_state_abbrev,
    ...
    abreviations=sorted(list(us_state_abbrev.values()))[0:10]
    state_names=states[-10:]
    return sorted(abreviations) + sorted(state_names)
